...
describe Keymaker::Node do
  ...
  describe "#save" do
    ...
    context "when the node is new" do
      let(:t800) { Terminator.new(name: 'T800') }
      it "creates and persists the new node" do
        t800.save
        t800.should be_persisted
      end
    end

    context "when the node is not new" do
      let(:t801) { Terminator.create(name: 'T801') }
      it "creates and persists the new node" do
        t801.should_not_receive(:create)
        t801.should_receive(:update_attributes)
        t801.name = "T1000"
        t801.save
      end
    end
