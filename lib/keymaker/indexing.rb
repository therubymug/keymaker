    ...
    def update_indices
      self.class.indices_traits.each do |index,traits|
        traits.each do |trait|
          neo_service.remove_node_from_index(index, trait[:index_key], send(trait[:value]), neo4j_id)
          neo_service.add_node_to_index(index, trait[:index_key], send(trait[:value]), neo4j_id)
        end
      end
    end
